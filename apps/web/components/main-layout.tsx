...
export default function MainLayout({
  ...
  const { user, activeTenant, memberships, switchTenant, isLoading } =
    useTenant();
  ...
  const userRole =
    user?.platform_role === "admin"
      ? "admin"
      : currentMembership?.role || "operator";
  const visibleDesktopItems = desktopMenuItems.filter((item) =>
  ...
  return (
    <div className="bg-[#EBEEF2] flex flex-col md:flex-row min-h-screen">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-[#1D3146]/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl animate-in zoom-in-95">
            <h2 className="text-2xl font-black text-[#1D3146] mb-2 tracking-tight">
              Nuevo Negocio Academy
            </h2>
            <p className="text-slate-500 text-sm mb-8 italic">
              Registra una nueva operación en la plataforma.
            </p>

            <form onSubmit={handleCreateBusiness} className="space-y-6">
              <div className="space-y-2">
                <label
                  htmlFor="modal_biz_name"
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button
                  type="submit"
                  disabled={creating}
                  className="flex-1 py-4 bg-[#1D3146] text-[#56CCF2] rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-[#1D3146]/20 disabled:opacity-50"
                >
      ...
      <aside className="hidden md:flex w-72 bg-[#1D3146] text-white flex-col fixed top-0 h-screen overflow-y-auto border-r border-slate-800 z-50">
        <div className="p-8 pb-8 flex flex-col items-center gap-10">
          ...
          >
            ...
            />
            <span className="text-[8px] font-black uppercase tracking-[0.3em] text-[#56CCF2]/40 -mt-2">
          ...
          <div className="relative w-full">
            ...
            >
              <img
                src={activeTenant?.logo_url || "/chocobites.jpg"}
                alt={activeTenant?.name}
                className="w-12 h-12 rounded-full border-2 border-white/20 shadow-md"
              />
              <div className="flex items-center gap-2 max-w-full">
                <span className="text-[11px] font-black uppercase tracking-widest text-[#56CCF2] truncate">
            ...
            {showTenantSwitcher && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#2B4764] border border-white/10 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-in slide-in-from-top-2">
                <div className="p-4 border-b border-white/5 bg-white/5">
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#56CCF2]">
                    Mis Negocios
                  </p>
                </div>
                <div className="max-h-60 overflow-y-auto">
                  ...
                  <div className="p-4 bg-white/5 border-t border-white/5">
                    <button
                      ...
                      className="text-xs text-[#56CCF2] font-black uppercase tracking-[0.1em] w-full text-left flex items-center justify-between group/add"
                    >
          ...
          <Link
            href="/operations"
            className="w-full py-4 bg-[#56CCF2] text-[#1D3146] rounded-2xl flex items-center justify-center gap-3 font-black uppercase text-xs tracking-widest shadow-lg shadow-[#56CCF2]/20 hover:scale-105 transition-all active:scale-95 group"
          >
            <PlusCircle
              size={20}
            ...
            >
              <item.icon
                ...
                className={
                  pathname === item.href
                    ? "text-[#56CCF2]"
                    : "text-slate-400 group-hover:text-[#56CCF2] transition-colors"
                }
              />
              {item.label}
            </Link>
        ...
        <div className="mt-auto p-6 border-t border-white/5 bg-white/2 cursor-pointer flex items-center justify-between group">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center font-black text-[10px]">
              {user?.full_name
                .split(" ")
                .map((n) => n[0])
                .join("")}
          ...
          >
            {item.primary ? (
              <div className="flex flex-col items-center">
                ...
                <span className="text-[10px] font-black uppercase tracking-tighter mt-1 text-[#1D3146]">
                  {item.label}
                </span>
              </div>
            ) : (
              <>
                ...
                />
                <span
                  className={`text-[10px] font-black uppercase tracking-tighter ${pathname === item.href ? "text-[#1D3146]" : "text-slate-400"}`}
                >
                  {item.label}
                </span>
              </>
      ...
      <main className="flex-grow pt-0 md:pl-72 pb-[120px] md:pb-0 min-h-screen relative">
        ...
        <div className="p-6 md:p-10 lg:p-12 pt-0 md:pt-4">{children}</div>
      </main>
    </div>
  );
}
