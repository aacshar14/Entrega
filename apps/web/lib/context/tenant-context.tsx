...
export function TenantProvider({ children }: { children: React.ReactNode }) {
  ...
  const [activeRole, setActiveRole] = useState<"owner" | "operator" | null>(
    null,
  );
  ...
  const fetchContext = useCallback(
    async (overrideId?: string, accessToken?: string) => {
      try {
        ...
        if (data.active_tenant?.id) {
          ...
          const m = data.memberships?.find(
            (m: any) => m.tenant.id === data.active_tenant.id,
          );
