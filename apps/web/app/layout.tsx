...
function UI_Shell({ children }) {
  ...
  const { user, activeTenant, activeRole, isLoading, clearTenant } =
    useTenant();
  ...
  const isOwnerOnlyRoute = tenantMenuItems.find(
    (i) => i.href === pathname,
  )?.ownerOnly;
  ...
  return (
    <div className="flex min-h-screen bg-[#EBEEF2] font-sans antialiased text-slate-900">
      ...
      {!isPublicPath && !isTenantSetupPath && (
        <aside
          className={`fixed inset-y-0 left-0 w-64 lg:w-60 xl:w-64 2xl:w-80 ${isPlatformPath ? "bg-[#0F172A]" : "bg-[#1D3146]"} text-white flex flex-col z-[70] transition-all duration-500 lg:translate-x-0 ${
      ...
      {/* Main Content Area */}
      <div
        className={`flex-grow ${!isPublicPath && !isTenantSetupPath ? "pl-0 lg:pl-60 xl:pl-64 2xl:pl-80" : "pl-0"} flex flex-col min-h-screen max-w-full overflow-x-hidden`}
      >
        {/* Top Header */}
        <header className="h-16 md:h-20 bg-white/70 backdrop-blur-md sticky top-0 z-40 px-4 md:px-8 lg:px-10 flex items-center justify-between border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div
              className={`${isPlatformPath ? "bg-amber-100" : "bg-[#56CCF2]/20"} px-2 py-0.5 md:px-3 md:py-1 rounded-full hidden sm:flex items-center gap-2 transition-colors flex-shrink-0`}
            >
              <ShieldCheck
                size={14}
              ...
              />
              <span
                className={`text-[9px] md:text-[10px] font-black uppercase ${isPlatformPath ? "text-amber-900" : "text-[#1D3146]"} tracking-tighter`}
              >
