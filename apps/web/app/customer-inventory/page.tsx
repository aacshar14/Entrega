...
export default function CustomerInventoryPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const productData = await apiRequest('products', 'GET', null, activeTenant.id);
      setProducts(productData || []);

      // 2. Fetch inventory summary
      const summaryData = await apiRequest('movements/customer-inventory/summary', 'GET', null, activeTenant.id);
  ...
  const fetchCustomerDetails = async (customerId: string, customerName: string) => {
    if (!activeTenant) return;
    try {
      setSelectedCustomer({ id: customerId, name: customerName });
      setDrilldownLoading(true);
      const allMovements = await apiRequest('movements', 'GET', null, activeTenant.id);
      const filtered = (allMovements || []).filter((m: any) => m.customer_id === customerId);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
        ...
        <div className="fixed inset-0 bg-[#1D3146]/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl border border-slate-100">
            <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
              <div>
                <h2 className="text-xl font-black text-[#1D3146]">{selectedCustomer.name}</h2>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Historial completo de movimientos</p>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-8 scrollbar-hide">
              {drilldownLoading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Consultando registros...</p>
                </div>
              ) : customerMovements.length === 0 ? (
                <div className="text-center py-20">
                  <p className="text-sm font-bold text-slate-300">No hay movimientos registrados para este cliente.</p>
                </div>
              ) : (
                    ...
                    <div key={move.id} className="group flex items-center justify-between p-6 bg-slate-50/50 hover:bg-white rounded-3xl border border-transparent hover:border-slate-100 transition-all hover:shadow-xl hover:shadow-slate-100/50">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                          move.type === 'delivery' || move.type === 'delivery_to_customer' ? 'bg-orange-50 text-orange-600' : 
                          move.type === 'sale_reported' ? 'bg-blue-50 text-blue-600' : 'bg-emerald-50 text-emerald-600'
                        }`}>
                          {move.type === 'delivery' || move.type === 'delivery_to_customer' ? <TrendingDown size={20} /> : 
                           move.type === 'sale_reported' ? <Layers size={20} /> : <RefreshCcw size={20} />}
                        </div>
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                             <span className="text-sm font-black text-[#1D3146]">
                               {move.type === 'delivery' || move.type === 'delivery_to_customer' ? 'Entrega' : 
                                move.type === 'sale_reported' ? 'Venta' : 'Devolución'}
                             </span>
                      ...
                      <div className="flex items-center gap-3">
                         <div className="text-right mr-2">
                           <p className="text-sm font-black text-[#1D3146]">${parseFloat(move.total_amount || 0).toLocaleString()}</p>
                           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tight">Importe</p>
                         </div>
                         <button 
            ...
            <div className="p-8 border-t border-slate-50 bg-slate-50/30">
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center">
                ⚠️ Las eliminaciones disparan una reconciliación automática de saldos.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-3xl font-black text-[#1D3146] tracking-tight">Resumen de Inventario</h1>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Último Mov.</th>
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                filtered.map((item) => (
                  <tr 
                    ...
                    onClick={() => fetchCustomerDetails(item.customer_id, item.customer_name)}
                    ...
                    {products.map(p => {
                      ...
                      return (
                        <td key={p.id} className="px-4 py-6 text-center">
                          <span className={`text-sm font-black ${qty > 0 ? 'text-[#1D3146]' : 'text-slate-200'}`}>
                            {qty || '-'}
                          </span>
                        </td>
                      );
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex flex-col items-end">
                        <span className="text-xs font-bold text-[#1D3146] flex items-center gap-2">
                          ...
                          {item.last_movement_at ? new Date(item.last_movement_at).toLocaleDateString('es-MX', { day: '2-digit', month: 'short' }) : 'N/A'}
                        </span>
                        <span className="text-[10px] text-slate-400 font-medium">
                          {item.last_movement_at ? new Date(item.last_movement_at).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' }) : '-'}
                        </span>
                      </div>
                    </td>
                  </tr>
      ...
      <div className="flex items-center justify-center pt-4">
        <p className="text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em] flex items-center gap-3">
          ...
          Haz clic en un cliente para ver el historial detallado de entregas y ventas
          <span className="w-1.5 h-1.5 bg-slate-200 rounded-full"></span>
        </p>
      </div>
    </div>
