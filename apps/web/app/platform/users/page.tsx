  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-[#1D3146] tracking-tight">Directorio de Usuarios</h1>
          <p className="text-slate-500 mt-2 font-medium">Gestión global de identidades y membresías.</p>
        </div>
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
         <div className="p-10 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
            ...
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{filteredUsers.length} de {users.length} Usuarios</p>
         </div>
         
         {isLoading ? (
           <div className="p-20 flex flex-col items-center justify-center gap-4">
              <div className="w-10 h-10 border-4 border-slate-100 border-t-blue-500 rounded-full animate-spin"></div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em]">Cargando usuarios...</p>
           </div>
         ) : (
                       ...
                       <tr key={user.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-6">
                             <div className="flex flex-wrap gap-2">
                                {user.memberships && user.memberships.length > 0 ? user.memberships.map((m: any, idx: number) => (
                                   ...
                                   <span className="text-[10px] font-bold text-slate-300 italic px-3 py-1 border border-dashed border-slate-200 rounded-full">Sin membresías</span>
                                )}
                             </div>
                          </td>
                          <td className="px-6 py-6">
