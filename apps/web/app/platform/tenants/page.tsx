      ...
      return (
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase text-emerald-600">Conectado</span>
          </div>
          <p className="text-[9px] font-bold text-slate-400 font-mono">{display || 'Meta Cloud API'}</p>
        </div>
      );
    }

    if (s === 'pending' || (tenant.onboarding_step && tenant.onboarding_step < 4)) {
       return (
      ...
      return (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase text-rose-600">Error ⚠️</span>
          </div>
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase text-slate-400 tracking-tighter italic">No Iniciado</span>
        </div>
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-medium italic mt-1">{memberships.length} negocios registrados.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
        {filtered.map(m => (
          <div key={m.tenant.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 p-10 group hover:border-[#56CCF2]/30 transition-all">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10">
              ...
              <div className="flex items-center gap-6 flex-1 min-w-[300px]">
                ...
                <div>
                  <h4 className="text-2xl font-black text-[#1D3146] tracking-tight">{m.tenant.name}</h4>
                  <p className="text-xs font-mono font-bold text-slate-400 uppercase tracking-tighter">{m.tenant.slug} • {m.tenant.id.slice(0,8)}</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-12 lg:flex-1 justify-between lg:justify-end">
                ...
                <div className="space-y-2 min-w-[140px]">
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">WhatsApp / Onboarding</p>
