          ...
          return (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl hover:scale-[1.02] transition-all">
              ...
              <div className="flex items-baseline gap-2">
                 ...
                 {stat.subtext && <span className="text-[10px] font-bold text-slate-400 italic whitespace-nowrap">{stat.subtext}</span>}
              </div>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  <thead>
                    <tr className="border-b border-slate-50">
                        ...
                        <th className="px-4 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Volumen</th>
                         ...
                         <th className="px-4 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Status</th>
                         <th className="px-4 py-4 text-[10px] font-black uppercase text-slate-400 tracking-widest text-center">Backlog</th>
                     </tr>
                  </thead>
                  <tbody>
                    {pressure.map((p, idx) => (
                      <tr key={idx} className="group hover:bg-slate-50/50 transition-colors border-b border-slate-50 last:border-0 font-medium text-xs">
                        <td className="px-6 py-5">
                           <div className="flex items-center gap-2">
                              ...
                              <span className="font-bold text-[#1D3146]">{p.tenant_name}</span>
                           </div>
                        </td>
                        <td className="px-4 py-5 text-center">
                           <span className="font-black text-slate-600">{p.volume_24h.toLocaleString()}</span>
                        </td>
                        <td className="px-4 py-5 text-center">
                           <span className={`font-black ${p.p95_processing_ms > 5000 ? 'text-amber-600' : 'text-slate-400'}`}>
                              {p.p95_processing_ms > 0 ? `${p.p95_processing_ms}ms` : '---'}
                           </span>
                        </td>
                        <td className="px-4 py-5 text-center">
                            <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${p.whatsapp_status === 'connected' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                               {p.whatsapp_status}
                            </span>
                        </td>
                        <td className="px-4 py-5 text-center">
        ...
        <div className="bg-[#0F172A] text-white rounded-[3rem] p-10 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-6 relative z-10">
               ...
               <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <span className="text-xs font-bold text-slate-400">Active Workers</span>
                  <span className="text-xs font-black uppercase tracking-widest text-emerald-400">
                     {queueStats?.active_processing ?? 0}
                  </span>
               </div>
               <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  <span className="text-xs font-bold text-slate-400">DB Utilization</span>
            ...
            <div className="mt-10 p-5 bg-white/5 rounded-2xl border border-white/10">
               ...
               <p className="text-xs text-slate-400 leading-relaxed font-medium">
                  {capacity?.advice || "Infrastructure operating within normal parameters."}
               </p>
            </div>
        </div>
      </div>
