...
export default function PlatformHealth() {
  ...
  const components = [
    {
      name: "API Gateway",
      status: healthData?.api_v1 || "operational",
      icon: Globe,
      color: "text-blue-500",
    },
    {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium">
            Estado en tiempo real de los servicios críticos de Entrega.
          </p>
        </div>
        <div className="flex items-center gap-2 px-6 py-3 bg-white rounded-2xl border border-slate-100 shadow-sm">
          <div
            className={`w-3 h-3 rounded-full ${healthData?.status === "online" ? "bg-emerald-500 animate-pulse" : "bg-slate-300"}`}
          ></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {components.map((comp, i) => (
          ...
          >
            ...
            <h3 className="font-bold text-[#1D3146] mb-1 relative">
              {comp.name}
            </h3>
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest relative">
              {comp.status}
            </p>
          </div>
        ))}
        ...
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/50 p-10">
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="font-mono text-sm font-bold text-[#1D3146]">
                {healthData?.version || "..."}
              </p>
            </div>
            <div>
