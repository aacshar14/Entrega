...
export default function CustomersPage() {
  ...
  const [step, setStep] = useState<"list" | "upload" | "preview" | "summary">(
    "list",
  );
  ...
  const [confirmingLiquidation, setConfirmingLiquidation] =
  ...
  const handleDeleteCustomer = async (id: string) => {
    if (!confirm("¿Estás seguro de eliminar este cliente?") || !activeTenant)
      return;
    try {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "https://api.entrega.space"}/api/v1/customers/import/preview`,
        {
          method: "POST",
          headers: {
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 relative min-h-screen pb-20">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewCustomer({ ...newCustomer, tier: e.target.value })
                    }
                  >
                    ...
                    >
                      <td className="pl-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-bold text-[#1D3146]">
                              {customer.name}
                            </p>
                      ...
                      <td className="px-6 py-5">
                        <span
                          className={`text-[10px] font-black uppercase px-2.5 py-1 rounded-lg ${
                            customer.tier === "mayoreo"
                              ? "bg-purple-100 text-purple-700"
                              : customer.tier === "especial"
                          ...
                          {activeMenu === customer.id && (
                            <div className="absolute right-8 top-12 z-20 w-44 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 animate-in fade-in slide-in-from-top-2 duration-200">
                              ...
                              {customer.balance && customer.balance < 0 && (
                                <button
                                  onClick={() => {
                                    setConfirmingLiquidation(customer);
                                    setActiveMenu(null);
                                  }}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#1D3146]/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
            <div className="p-10 text-center">
              ...
              <h2 className="text-3xl font-black text-[#1D3146] mb-4 tracking-tighter">
                ¡Liquidación VIP!
              </h2>
              <p className="text-slate-500 font-medium leading-relaxed mb-8">
                ¿Confirmas la recepción de{" "}
                <span className="text-emerald-600 font-black">
                  $
                  {Math.abs(
                    confirmingLiquidation.balance || 0,
                  ).toLocaleString()}
                </span>{" "}
                para saldar la cuenta de{" "}
                <span className="text-[#1D3146] font-bold">
                  {confirmingLiquidation.name}
                </span>
                ?
                <br />
                <br />
                <span className="text-[10px] uppercase tracking-widest font-black text-slate-400">
                  Esta acción liberará el inventario en tránsito.
                </span>
              </p>
      ...
      {step === "upload" && (
        <div className="max-w-xl mx-auto bg-white rounded-3xl border border-slate-200 shadow-2xl p-10 mt-10">
          ...
          <p className="text-slate-500 mb-8 font-medium">
            Carga tu archivo CSV con el formato de la plantilla.
          </p>
      ...
      {step === "preview" && preview && (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl overflow-hidden mt-10 animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div>
              <h2 className="text-xl font-bold text-[#1D3146]">
                Vista Previa de Importación
              </h2>
            </div>
            <div className="flex gap-4 text-[10px] font-black uppercase tracking-widest">
              <div className="bg-green-100 text-green-700 px-4 py-2 rounded-xl">
                Válidos: {preview.valid_rows_count}
              </div>
              <div className="bg-red-100 text-red-700 px-4 py-2 rounded-xl">
                Errores: {preview.invalid_rows_count}
              </div>
              <div className="bg-amber-100 text-amber-700 px-4 py-2 rounded-xl">
                Duplicados: {preview.duplicate_rows_count}
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                {preview.rows.map((row, idx) => (
                  <tr
                    key={idx}
                    className={`hover:bg-slate-50/80 ${!row.is_valid ? "bg-red-50/30" : ""}`}
                  >
                    <td className="px-8 py-4 text-slate-400 font-mono text-xs">
                      #{row.row_index}
                    </td>
                    <td className="px-4 py-4 font-bold text-[#1D3146] text-sm">
                      {row.data.name}
                    </td>
      ...
      {step === "summary" && summary && (
        <div className="max-w-xl mx-auto bg-white rounded-3xl border border-slate-200 shadow-2xl p-10 text-center mt-10 animate-in zoom-in-95">
          ...
          <h2 className="text-3xl font-black text-[#1D3146] tracking-tight">
            ¡Importación Exitosa!
          </h2>
          <p className="text-slate-500 mt-2 mb-10 font-medium">
            Se han procesado {summary.created} registros nuevos.
          </p>
