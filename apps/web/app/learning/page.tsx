...
export default function LearningDashboard() {
      ...
      setLogs([
        {
          ...
          raw_message:
            "Entregar 10 productos a Distribuidora Los Santos mañana",
          timestamp: new Date().toISOString(),
          detected_intent: "delivery",
          detected_entities: JSON.stringify({
            customer: "Distribuidora Los Santos",
            qty: 10,
            product: "Producto A",
          }),
          confidence: 0.92,
  ...
  const handleAddAlias = async () => {
    ...
    try {
      await apiRequest(
        `/learning/aliases/${newAlias.type}`,
        "POST",
        {
          [newAlias.type === "customer" ? "customer_id" : "product_id"]:
            newAlias.id,
  ...
  if (loading)
    return (
      <div className="p-20 text-center text-slate-400">
        Cargando inteligencia del negocio...
      </div>
    );
  ...
  return (
    <div className="space-y-10 pb-20 animate-in fade-in duration-500">
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 font-medium italic underline decoration-[#56CCF2]/30">
            Entrenando el parsing engine con lenguaje real de{" "}
            {activeTenant?.name}.
          </p>
        </div>
      </section>
      ...
      <div className="flex gap-4 border-b border-slate-100">
        <button
          onClick={() => setTab("logs")}
          className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all ${tab === "logs" ? "text-[#1D3146] border-b-2 border-[#1D3146]" : "text-slate-300"}`}
        >
          Message Review
        </button>
        <button
          onClick={() => setTab("dictionary")}
          className={`pb-4 px-2 text-xs font-black uppercase tracking-widest transition-all ${tab === "dictionary" ? "text-[#1D3146] border-b-2 border-[#1D3146]" : "text-slate-300"}`}
        >
          Tenant Dictionary
        </button>
      </div>
          ...
          <div className="lg:col-span-12 space-y-4">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                Mensajes del Centro de Control
              </h3>
              <button
                onClick={fetchLogs}
                title="Refrescar logs"
                className="p-2 text-slate-400 hover:text-[#1D3146]"
              >
                <RefreshCcw size={16} />
              </button>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-slate-50">
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-[#1D3146]/40">
                        Origen
                      </th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-[#1D3146]/40">
                        Mensaje Raw
                      ...
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-[#1D3146]/40 text-right">
                        Acción
                      </th>
                    </tr>
                  </thead>
                      ...
                      >
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs font-bold text-[#1D3146]">
                              {log.sender}
                            </span>
                          </div>
                        </td>
                        <td className="px-8 py-5 max-w-xs">
                          <p className="text-xs font-medium text-slate-600 italic truncate">
                            "{log.raw_message}"
                          </p>
                        </td>
                        <td className="px-8 py-5">
                        ...
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-2">
                            <div className="flex-grow w-12 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                              <div
                                className={`h-full transition-all ${log.confidence > 0.8 ? "bg-emerald-400" : log.confidence > 0.5 ? "bg-orange-400" : "bg-rose-400"}`}
                          ...
                          <button
                            onClick={() => {
                              ...
                              setCorrection({
                                ...
                                entities: JSON.parse(log.detected_entities),
                              });
                            }}
          ...
          <div className="lg:col-span-12 space-y-8 animate-in slide-in-from-bottom-4">
            <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center gap-10">
              <div className="flex-grow space-y-6">
                <h3 className="text-xl font-black text-[#1D3146] tracking-tight">
                  Agregar Alias al Diccionario
                </h3>
                <p className="text-sm text-slate-500 italic">
                  Mapea nombres informales de WhatsApp a entidades reales del
                  sistema.
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <select
                    ...
                    value={newAlias.type}
                    onChange={(e) =>
                      setNewAlias({ ...newAlias, type: e.target.value })
                    }
                  ...
                  <input
                    ...
                    value={newAlias.id}
                    onChange={(e) =>
                      setNewAlias({ ...newAlias, id: e.target.value })
                    }
                ...
                <button
                  ...
                  className="px-10 py-5 bg-[#1D3146] text-[#56CCF2] font-black rounded-2xl shadow-lg flex items-center gap-3 uppercase text-xs tracking-widest hover:scale-105 active:scale-95 transition-all"
                >
          ...
          <div className="bg-white rounded-[2.5rem] p-10 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-slate-500 text-sm italic">
                  Corrige y enseña al parser de {activeTenant?.name}.
                </p>
              </div>
              <button
            ...
            <div className="space-y-8">
              <div className="bg-[#EBEEF2] p-8 rounded-3xl border border-slate-200">
                ...
                <p className="text-lg font-bold text-[#1D3146]">
                  "{reviewing.raw_message}"
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCorrection({ ...correction, intent: e.target.value })
                    }
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    {["pending", "processed", "corrected", "failed"].map(
                      (s) => (
                        <button
                          key={s}
              ...
              <div className="p-8 bg-slate-50 border border-slate-100 rounded-3xl space-y-6">
                ...
                <pre className="text-xs bg-white p-4 rounded-xl border border-slate-200 text-[#1D3146] font-mono">
                  {JSON.stringify(
                    JSON.parse(reviewing.detected_entities),
                    null,
                    2,
                  )}
                </pre>
                <p className="text-[10px] text-slate-400 italic">
                  Si el cliente o producto es incorrecto, agrega un alias en la
              ...
              <div className="flex gap-4 pt-4">
                <button
                  onClick={() => setReviewing(null)}
                  className="flex-1 py-5 text-slate-400 font-bold uppercase text-[10px] tracking-widest"
                >
                  Descartar
                </button>
                <button
                  onClick={() => handleApprove(reviewing.id)}
                  className="flex-[2] py-5 bg-[#1D3146] text-[#56CCF2] rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl shadow-[#1D3146]/20 flex items-center justify-center gap-3"
                >
