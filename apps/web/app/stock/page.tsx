...
export default function StockPage() {
  ...
  // Actions state
  const [editingProduct, setEditingProduct] = useState<ProductStock | null>(
    null,
  );
  ...
  const handleDeleteProduct = async (id: string) => {
    if (
      !confirm("¿Seguro que quieres eliminar este producto del catálogo?") ||
      !activeTenant
    )
      return;
    try {
  ...
  const saveBulkEdit = async () => {
    ...
    try {
      const promises = Object.values(bulkData).map((p) => {
        ...
        const adjustment = original
          ? Number(p.quantity) - original.quantity
          : 0;
  ...
  const handleCommitImport = async () => {
    ...
    try {
      ...
      setMessage(
        `✅ Catálogo actualizado: ${validRows.length} productos procesados.`,
      );
      setStep("list");
      setPreviewData(null);
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 h-full flex flex-col">
        <div className="flex items-center justify-between mb-8">
          <button
            ...
            className="flex items-center gap-2 text-slate-500 hover:text-[#1D3146] font-bold"
          >
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 flex-grow flex flex-col overflow-hidden">
          <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            <div>
              <h2 className="text-2xl font-black text-[#1D3146]">
                Vista Previa
              </h2>
            </div>
            <button
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {previewData.rows.map((row, idx) => (
                  <tr
                    key={idx}
                    className={`hover:bg-slate-50 ${!row.is_valid ? "bg-red-50/30" : ""}`}
                  >
            ...
            <form onSubmit={handleUpdateProduct} className="p-8 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-[10px] uppercase font-black text-slate-400 tracking-widest mb-2 px-2">
                    Nombre del Producto
                  </label>
                  <input
                    type="text"
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingProduct({
                          ...editingProduct,
                          price_menudeo: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingProduct({
                          ...editingProduct,
                          price_mayoreo: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingProduct({
                          ...editingProduct,
                          price_especial: Number(e.target.value),
                        })
                      }
                    />
                  </div>
                ...
                <div className="md:col-span-2 bg-[#56CCF2]/5 p-6 rounded-3xl border border-[#56CCF2]/10">
                  <label className="block text-[10px] uppercase font-black text-[#56CCF2] tracking-widest mb-3 px-2">
                    Ajustar Unidades (Stock)
                  </label>
                  <div className="flex items-center gap-4">
                    <div className="flex-grow">
                      <input
                        ...
                        value={
                          (editingProduct as any).adjustment_quantity || ""
                        }
                        onChange={(e) =>
                          setEditingProduct({
                            ...editingProduct,
                            adjustment_quantity: Number(e.target.value),
                          } as any)
                        }
                      />
                    </div>
                  ...
                  <p className="mt-2 text-[10px] font-medium text-slate-400 px-2 italic">
                    Ingresa un valor positivo para sumar o negativo para restar
                    del stock actual.
                  </p>
                </div>
              </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-black text-[#1D3146] tracking-tighter flex items-center gap-4">
            ...
            >
              <RefreshCcw
                size={16}
                className={loading && message === null ? "animate-spin" : ""}
              />
            </button>
          </h1>
        </div>
        <div className="flex gap-4">
          {!isBulkEditing ? (
            <>
              ...
              <label
                className={`cursor-pointer flex items-center gap-2 px-6 py-3 bg-[#1D3146] text-white text-sm font-bold rounded-2xl hover:scale-105 transition-all shadow-xl shadow-[#1D3146]/20 ${uploading ? "opacity-50 pointer-events-none" : ""}`}
              >
                {uploading ? (
                  <Loader2 className="animate-spin" />
                ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-[#1D3146] text-white sticky top-0 z-10">
              <tr className="text-[10px] font-black uppercase tracking-widest text-white/70">
                ...
                <th className="px-4 py-5 text-right font-black text-[#56CCF2]">
                  Mayoreo
                </th>
                <th className="px-4 py-5 text-right font-black text-[#F2C94C]">
                  Especial
                </th>
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="p-20 text-center">
                    <Loader2
                      className="animate-spin mx-auto text-[#56CCF2]"
                      size={32}
                    />
                  </td>
                </tr>
                ...
                filteredProducts.map((p) => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`group hover:bg-slate-50 transition-all font-medium ${isEditingRow ? "bg-[#56CCF2]/5" : ""}`}
                    >
                      <td className="px-8 py-5">
                        {isEditingRow ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleBulkChange(p.id, "name", e.target.value)
                            }
                          />
                        ) : (
                          <span className="font-bold text-[#1D3146] group-hover:text-[#56CCF2] transition-colors">
                            {p.name}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-5">
                        {isEditingRow ? (
                          <input
                            ...
                            onChange={(e) =>
                              handleBulkChange(p.id, "sku", e.target.value)
                            }
                          />
                        ) : (
                      ...
                      <td className="px-4 py-5 text-center">
                        {isEditingRow ? (
                          ...
                          <span
                            className={`px-3 py-1 rounded-xl text-[10px] font-black ${p.quantity > 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                          >
                            {p.quantity}
                          </span>
                        )}
                      ...
                      <td className="px-4 py-5 text-right">
                        {isEditingRow ? (
                          ...
                          <span className="font-black text-[#1D3146]">
                            ${p.price_menudeo.toFixed(2)}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="pr-8 py-5 text-right relative">
                        {!isBulkEditing ? (
                          ...
                          <span className="text-[10px] font-black text-slate-200 uppercase tracking-widest italic pr-2">
                            Editando...
                          </span>
                        )}
                      </td>
