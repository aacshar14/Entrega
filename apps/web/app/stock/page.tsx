...
export default function StockPage() {
  ...
  const fetchStock = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await apiRequest('/products/stock', 'GET');
      setProducts(data || []);
    } catch (err: any) {
      console.error('Error fetching stock:', err);
      // Don't show full error for 401 as it's handled by provider
      if (err.status !== 401) {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // For FormData, we use fetch directly but with the SSR token
      ...
      if (response.ok) {
        ...
        throw new Error(errData.detail || 'Error al subir el catálogo');
      }
    } catch (err: any) {
    ...
    <div className="max-w-7xl mx-auto py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
         ...
         <div className="flex gap-4">
            <label className={`cursor-pointer flex items-center gap-2 px-6 py-3 bg-[#1D3146] text-white text-sm font-bold rounded-2xl hover:scale-105 transition-all shadow-xl shadow-[#1D3146]/20 ${uploading ? 'opacity-50' : ''}`}>
               ...
               <input type="file" className="hidden" accept=".csv" aria-label="Seleccionar archivo CSV de inventario" onChange={handleFileUpload} disabled={uploading} />
            </label>
         </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/40 border border-slate-100">
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            ...
            <div className="flex gap-4">
               <button className="p-3 bg-[#EBEEF2] rounded-xl text-slate-500 hover:bg-slate-200 transition-colors" aria-label="Filtrar">
                  <Filter size={18} />
               </button>
               <a 
                 href="#" 
