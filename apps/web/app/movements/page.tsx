...
export default function MovementsPage() {
  ...
  const filteredMovements = movements.filter((m) => {
    ...
    return (
      sku.includes(search) || desc.includes(search) || type.includes(search)
    );
  });
                ...
                filteredMovements.map((movement) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <div
                          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-sm ${
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold text-[#1D3146]">
                            {movement.customer_name_snapshot || "—"}
                          </span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-3">
                          <span
                            className={`text-sm font-black ${isPositive ? "text-emerald-600" : "text-rose-600"}`}
                          >
                            {isPositive ? "+" : ""}
                            {movement.quantity}
                          </span>
                          ...
                          {movement.tier_applied && (
                            <span className="text-[8px] bg-slate-100 px-1.5 py-0.5 rounded-md font-black uppercase tracking-tighter text-slate-500">
                              {movement.tier_applied}
                            </span>
                          )}
                        </div>
