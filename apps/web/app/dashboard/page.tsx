...
const safeMoney = (value: unknown): string =>
...
interface DashboardData {
  ...
  stock?: Array<StockItem> | null;
  debtors?: Array<{ name?: string; amount?: number }> | null;
...
export default function Dashboard() {
  ...
  // --- Data Normalization (Anti-Crash Layer) ---
  const stats = data?.stats;
  const billing = data?.billing;
  const stockItems = safeArray(data?.stock);
  const recentActivity = safeArray(data?.recent_activity);
  const debtorsList = safeArray(data?.debtors);
  ...
  const billingExpired = Boolean(billing?.is_expired);
  const totalOrders = safeNumber(billing?.total_orders);
  const salesToday = safeNumber(billing?.sales_today);

  const welcomeMessage = safeText(data?.welcome_message, "Bienvenido");
  const businessName = safeText(data?.business_name, "Entrega");
  const firstName = safeText(user?.full_name, "Admin").split(" ")[0] || "Admin";
  const tenantName = safeText(activeTenant?.name, "tu negocio");

  const totalInWarehouse = stockItems.reduce((sum, item) => sum + safeNumber(item?.quantity), 0);
  const totalOutside = stockItems.reduce((sum, item) => sum + safeNumber(item?.quantity_outside), 0);
  const totalInventory = stockItems.reduce((sum, item) => sum + safeNumber(item?.total), 0);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500 px-4 md:px-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-purple-100 text-sm font-medium">
                Te quedan {billingDaysRemaining}{" "}
                {billingDaysRemaining === 1 ? "día" : "días"} para
        ...
        billingDaysRemaining <= 3 && (
          <div className="p-6 rounded-[2rem] bg-amber-500 text-white shadow-xl shadow-amber-500/20 flex items-center justify-between gap-6 border-2 border-amber-300/30">
            <p className="font-bold flex items-center gap-3">
              ...
              {billingDaysRemaining === 1 ? "día" : "días"}.
            </p>
            <Link
              href="/onboarding"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {totalOrders >= 5 && (
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center gap-4 text-emerald-700 animate-in zoom-in duration-500">
              ...
              <p className="text-sm font-black uppercase tracking-tight">
                Ya estás usando Entrega para vender ({totalOrders}{" "}
                envíos)
              </p>
            </div>
          )}
          {salesToday > 0 && (
            <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex items-center gap-4 text-blue-700 animate-in zoom-in duration-700">
              ...
              <p className="text-sm font-black uppercase tracking-tight">
                Hoy generaste ${safeMoney(salesToday)} con
                Entrega
              </p>
            </div>
          )}
      ...
      <div
        className={`space-y-8 relative ${billingExpired ? "overflow-hidden rounded-[3rem]" : ""}`}
      >
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            <span className="text-[10px] bg-orange-500 text-white px-3 py-1 rounded-full uppercase tracking-widest shrink-0 font-bold">
              {DASHBOARD_VERSION}
            </span>
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="rounded-3xl p-7 bg-gradient-to-br from-rose-500 to-rose-600 text-white flex flex-col justify-between h-44 shadow-2xl shadow-rose-500/20 transition-transform hover:scale-[1.02]">
            ...
            <p className="text-[11px] font-bold opacity-80 uppercase tracking-widest italic flex items-center gap-1">
              <AlertCircle size={12} /> {debtorCount} Cuentas por Cobrar
            </p>
          </div>
                      ...
                      {stockItems.map((item, idx) => (
                        ...
                        >
                          ...
                          <td className="py-4 text-center">
                            <span
                              className={`${safeNumber(item.quantity) <= 10 ? "text-orange-600" : "text-slate-600"} font-medium`}
                            >
                              {safeNumber(item.quantity)}
                            </span>
                          </td>
                          <td className="py-4 text-center text-slate-400 font-medium">
                            {safeNumber(item.quantity_outside) > 0 ? (
                              <span className="text-blue-500">
                                +{safeNumber(item.quantity_outside)}
                              </span>
                            ) : (
                              "0"
                            )}
                          ...
                          <td className="py-4 text-right font-black text-slate-900">
                            {safeMoney(safeNumber(item.quantity) + safeNumber(item.quantity_outside))}
                          </td>
                        </tr>
                      ))}
                    </tbody>
