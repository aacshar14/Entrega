...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(`${origin}/settings/integrations/whatsapp?error=${error}`);
  }

  if (code) {
    // Redirect back to the WhatsApp integration page with the code
    // The page will handle the backend exchange automatically
    return NextResponse.redirect(`${origin}/settings/integrations/whatsapp?code=${code}`);
  }
