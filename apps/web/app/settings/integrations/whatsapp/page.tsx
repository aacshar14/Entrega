...
export default function WhatsAppConfigPage() {
  ...
  useEffect(() => {
    ...
    if (!window.FB) {
      ...
      script.onload = () => {
        const appId =
          process.env.NEXT_PUBLIC_META_APP_ID || META_APP_ID_FALLBACK;
        if (window.FB) {
          window.FB.init({
            appId: appId,
            cookie: true,
  ...
  const handleLaunchOnboarding = async () => {
    ...
    try {
      ...
      window.FB.login(
        ...
        {
          config_id:
            process.env.NEXT_PUBLIC_META_CONFIG_ID || META_CONFIG_ID_FALLBACK,
          response_type: "code",
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-10 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-500 font-medium italic">
            Automatización nativa de pedidos vía Cloud API.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {/* Main Status Card */}
        <div
          className={`rounded-[3rem] p-10 border transition-all ${
            ...
            <div className="flex-1 space-y-6">
              <div className="flex items-center gap-3">
                ...
                {status?.last_validated && (
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter italic">
                    Validado:{" "}
                    {new Date(status.last_validated).toLocaleDateString()}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-4">
                <h3 className="text-4xl font-black text-[#1D3146] leading-tight">
                  {status?.status === "connected"
                    ? "Tu negocio ya está en piloto automático"
                    : status?.status === "token_expired"
            ...
            <div className="w-full md:w-72 space-y-4">
              <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-4">
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-mono font-bold text-[#1D3146] truncate">
                      {status?.phone_number_id || "---"}
                    </p>
                  </div>
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-[#1D3146]">
              ...
              <div className="p-6 bg-[#1D3146]/5 rounded-3xl space-y-3">
                ...
                <p className="text-[11px] font-medium text-slate-500 leading-snug">
                  Asegúrate de tener un método de pago válido en tu Meta
                  Business Center para evitar interrupciones.
                </p>
                <a
                  href="https://business.facebook.com/"
                  target="_blank"
                  className="flex items-center gap-2 text-[#56CCF2] text-[9px] font-black uppercase tracking-widest hover:underline"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
            ...
            <h4 className="text-xl font-black text-[#1D3146]">
              Privacidad de Datos
            </h4>
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              Tus conversaciones solo son procesadas por nuestra IA para extraer
              pedidos. Nunca guardamos el historial completo ni usamos tus datos
              para otros fines.
            </p>
          </div>
          ...
          <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
            ...
            <p className="text-sm text-slate-500 font-medium leading-relaxed">
              Somos Tech Providers oficiales. La conexión se realiza
              directamente mediante el <b>Embedded Signup</b> oficial de Meta,
