  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] gap-4">
        ...
        <p className="text-slate-500 font-medium animate-pulse">Cargando tus accesos...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-6">
      <div className="mb-12 text-center">
        <h1 className="text-4xl font-black text-slate-800 tracking-tight mb-3">Selector de Negocio</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {memberships.map((membership) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center group-hover:bg-blue-50 transition-colors">
                {membership.tenant.logo_url ? (
                  <img src={membership.tenant.logo_url} alt={membership.tenant.name} className="w-10 h-10 object-contain" />
                ) : (
            ...
            <div>
              <h3 className="text-2xl font-black text-slate-800 mb-1">{membership.tenant.name}</h3>
              <p className="text-slate-400 font-medium">slug: {membership.tenant.slug}</p>
            </div>
        ...
        <button className="group bg-slate-50 p-8 rounded-3xl border-2 border-dashed border-slate-200 hover:border-blue-400 hover:bg-white transition-all text-left flex flex-col items-center justify-center gap-4 min-h-[240px]">
          ...
          <div className="text-center">
             <h3 className="text-lg font-bold text-slate-500 group-hover:text-blue-600 transition-colors">Registrar Nuevo Tenant</h3>
      ...
      <div className="mt-16 p-8 bg-blue-50 rounded-3xl border border-blue-100 flex items-center gap-6">
        ...
        <div>
           <h4 className="font-bold text-blue-900">Búsqueda Global de Operaciones</h4>
           <p className="text-blue-700/70 text-sm">Como Super Admin, puedes buscar pedidos y folios a través de todos los tenants registrados.</p>
        </div>
      </div>
    </div>
  );
