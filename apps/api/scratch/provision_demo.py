...
def provision_demo():
    with Session(engine) as session:
        ...
        tenant = session.exec(select(Tenant).where(Tenant.slug == 'aguachiles-demo-final')).first()
        if not tenant:
        ...
        # Create membership
        membership = session.exec(select(TenantUser).where(TenantUser.user_id == user.id, TenantUser.tenant_id == tenant.id)).first()
        if not membership:
