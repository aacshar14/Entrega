...
=======
        memberships = session.exec(select(TenantUser).where(TenantUser.user_id == user.id)).all()
        print(f"Found {len(memberships)} memberships:")
        for m in memberships:
            t = session.get(Tenant, m.tenant_id)
            print(f"  - Tenant: {t.name if t else 'MISSING'} (Active: {m.is_active}, Default: {m.is_default})")
>>>>>>> 097c7426157a1f10be477ea1e49ee8392e2e50d2
