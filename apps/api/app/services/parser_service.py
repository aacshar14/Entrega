...
class ParserService:
    ...
    def parse_intent(text: str) -> Tuple[Intent, Dict[str, Any]]:
        ...
        if any(kw in text for kw in ["entrega", "entregue", "lleve", "venta"]):
            ...
            if "amargo" in text: entities["product"] = "amargo"
            elif "leche" in text: entities["product"] = "leche"
    ...
    async def process_message(db: Session, whatsapp_msg: WhatsAppMessage):
        ...
        logger.info("Processing business logic from message", sender=whatsapp_msg.from_number, body=whatsapp_msg.body)
        ...
        statement = select(Customer).where(Customer.phone_number == whatsapp_msg.from_number)
        customer = db.exec(statement).first()
        
        if not customer:
            logger.warning("Message from unregistered number", phone=whatsapp_msg.from_number)
        ...
        if intent == Intent.DELIVERY:
            ...
            if product:
                ...
                if not balance:
                    balance = CustomerBalance(tenant_id=tenant_id, customer_id=customer.id, balance=0.0)
                ...
                logger.info("Delivery registered successfully", customer=customer.name, product=product.name, cost=total_cost)
            ...
            logger.info("Payment registered successfully", customer=customer.name, amount=amount)
