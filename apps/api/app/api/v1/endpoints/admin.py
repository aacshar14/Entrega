...
async def get_queue_stats(db: Session = Depends(get_session)):
    ...
    processing = db.exec(
        select(func.count(InboundEvent.id)).where(InboundEvent.status == "processing")
    ).one()
    failed = db.exec(
        select(func.count(InboundEvent.id)).where(InboundEvent.status == "failed")
    ).one()
    done = db.exec(
        select(func.count(InboundEvent.id)).where(InboundEvent.status == "done")
    ).one()
...
async def get_capacity_advisor(db: Session = Depends(get_session)):
    ...
    total_active = db.exec(
        select(func.count(InboundEvent.id)).where(
            InboundEvent.created_at > text("now() - interval '24 hours'")
        )
    ).one()
    pending = db.exec(
        select(func.count(InboundEvent.id)).where(InboundEvent.status == "pending")
...
async def update_user_status(
    user_id: UUID, is_active: bool, db: Session = Depends(get_session)
):
    """
...
async def get_platform_costs(db: Session = Depends(get_session)):
    ...
    return {
        ...
        "breakdown": [
            ...
            {
                "label": "Event Processing (Trend Est.)",
                "value": round(events_24h * 30 * 0.001, 2),
                "unit": "USD",
            },
...
@router.put(
    "/settings/{key}", dependencies=[Depends(require_platform_role(["admin", "owner"]))]
)
...
async def simulate_demo_message(
    ...
    new_msg = WhatsAppMessage(
        ...
        phone_number_id="demo_simulator",
        sender_wa_id=request.from_number,
        message_sid=msg_id,
