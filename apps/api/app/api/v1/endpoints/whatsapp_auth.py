...
async def exchange_meta_code(
    ...
    if not settings.WHATSAPP_APP_ID or not settings.WHATSAPP_APP_SECRET:
        logger.error(
            "Missing WHATSAPP_APP_ID or WHATSAPP_APP_SECRET in environment configuration."
        )
        raise HTTPException(
            status_code=500,
            detail="Server is missing required Meta API configurations.",
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if fb_response.status_code != 200:
                logger.error(
                    "Meta token exchange failed", status_code=fb_response.status_code
                )
                raise HTTPException(
                    status_code=400, detail="Meta could not validate the provided code"
                )
            ...
            logger.info(
                "WhatsApp configuration successfully hardened and saved",
                tenant_id=str(active_tenant_id),
            )
        ...
        raise HTTPException(
            status_code=500, detail="An internal error occurred during WhatsApp setup"
        )
