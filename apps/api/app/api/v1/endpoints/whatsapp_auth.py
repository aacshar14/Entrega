...
async def setup_whatsapp_manual(
    ...
    # RBAC Check: We trust the active_tenant_id from the dependency
    # which is resolved from the session/token context.
    ...
    if existing_other:
        logger.warning("whatsapp_auth.manual_setup_collision", 
                       tenant_id=str(active_tenant_id),
