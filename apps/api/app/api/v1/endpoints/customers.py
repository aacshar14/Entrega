...
async def create_customer(
    ...
    new_customer = Customer(
        ...
        tier=(
            customer_data.tier
            if customer_data.tier in ["mayoreo", "menudeo", "especial"]
            else "menudeo"
        ),
...
async def import_customers_preview(
    ...
    for i, row in enumerate(reader, 1):
        ...
        if name.lower() in existing_names or (
            normalized_phone and normalized_phone in existing_phones
        ):
            is_duplicate = True
            duplicate_count += 1
        ...
        rows_preview.append(
            ImportPreviewRow(
                ...
                data=CustomerImportRow(
                    ...
                    tier=(
                        tier
                        if tier in ["mayoreo", "menudeo", "especial"]
                        else "menudeo"
                    ),
                ),
                is_valid=is_valid,
    ...
    for row in request.rows:
        ...
        existing = db.exec(
            select(Customer).where(
                ...
                & (
                    (func.lower(Customer.name) == row.name.lower())
                    | (Customer.phone_number == row.phone)
                )
            )
        ).first()
        ...
        if existing:
            ...
            existing.tier = (
                row.tier
                if row.tier in ["mayoreo", "menudeo", "especial"]
                else "menudeo"
            )
            ...
            balance_record = db.exec(
                select(CustomerBalance).where(
                    CustomerBalance.customer_id == existing.id
                )
            ).first()
...
async def update_customer(
    ...
    if update_data.tier is not None and update_data.tier in [
        "mayoreo",
        "menudeo",
        "especial",
    ]:
...
async def delete_customer(
    ...
    db.execute(
        text("DELETE FROM customer_balances WHERE customer_id = :id"), {"id": id}
    )
    db.execute(text("DELETE FROM customer_aliases WHERE customer_id = :id"), {"id": id})
    db.execute(
        text(
            "UPDATE inventory_movements SET customer_id = NULL WHERE customer_id = :id"
        ),
        {"id": id},
    )
