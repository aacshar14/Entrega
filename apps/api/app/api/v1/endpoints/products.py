...
async def list_products_stock(
    ...
    statement = (
        ...
        .join(StockBalance, Product.id == StockBalance.product_id, isouter=True)
...
async def import_products_preview(
    ...
    for i, row in enumerate(reader, 1):
        ...
        price_mayoreo_str = (
            row.get("price_mayoreo") or row.get("Mayoreo") or "0"
        ).strip()
        price_menudeo_str = (
            row.get("price_menudeo") or row.get("Menudeo") or "0"
        ).strip()
        price_especial_str = (
            row.get("price_especial") or row.get("Especial") or "0"
        ).strip()
        aliases_str = (
            row.get("aliases") or row.get("Alias") or row.get("Keywords") or ""
        ).strip()
    ...
    for row in request.rows:
        ...
        product = db.exec(
            select(Product).where(
                Product.tenant_id == tenant_id, Product.name == row.name
            )
        ).first()
        ...
        if not product and row.sku:
            product = db.exec(
                select(Product).where(
                    Product.tenant_id == tenant_id, Product.sku == row.sku
                )
            ).first()
            ...
            for word in alias_list:
                if word and word not in existing_alias_words:
                    new_alias = ProductAlias(
                        tenant_id=tenant_id, product_id=product.id, alias=word
                    )
                    db.add(new_alias)
...
async def update_product(
    ...
    # Stock Adjustment
    if (
        update_data.adjustment_quantity is not None
        and update_data.adjustment_quantity != 0
    ):
        balance = db.exec(
            select(StockBalance).where(StockBalance.product_id == product.id)
        ).first()
...
async def delete_product(
    ...
    db.execute(
        text("DELETE FROM inventory_movements WHERE product_id = :id"), {"id": id}
    )
