...
def seed_data():
    with Session(engine) as session:
        ...
        if not tenant:
            ...
            tenant = Tenant(id=uuid4(), name="ChocoBites", slug="chocobites")
            session.add(tenant)
            session.commit()
        ...
        for u_data in users_to_seed:
            ...
            if not membership:
                print(
                    f"🔗 Linking {user.email} to {tenant.slug} as {u_data['tenant_role']}..."
                )
        ...
        customer_stmt = select(Customer).where(
            Customer.phone_number == "+5211234567890"
        )
        ...
        if not customer:
            ...
            balance = CustomerBalance(
                tenant_id=tenant.id, customer_id=customer.id, balance=0.0
            )
            session.add(balance)
        else:
        ...
        if not product:
            ...
            # Initial stock
            stock = StockBalance(
                tenant_id=tenant.id, product_id=product.id, quantity=100.0
            )
            session.add(stock)
        else:
