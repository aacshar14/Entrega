    ...
    def _process_single_event(self, event: InboundEvent):
        ...
        if event.source == "whatsapp" and event.event_type == "message":
            ...
            try:
                ...
                result = self.db.execute(reclaim_sql, {
                    "now": now,
                    "tenant_id": tenant.id,
                    "message_id": msg_id,
                    "threshold": threshold
            ...
            try:
                ...
                """)
                result = self.db.execute(success_sql, {
                    "now": datetime.now(timezone.utc),
                    "tenant_id": tenant.id,
                    "message_id": msg_id
                })
                ...
                self.db.rollback()
                logger.error("idempotency.logic_failed", message_id=msg_id, error=str(e))
