...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_processed_messages_status'), 'processed_messages', ['status'], unique=False)
    op.create_index(op.f('ix_processed_messages_tenant_id'), 'processed_messages', ['tenant_id'], unique=False)
    op.create_index('idx_processed_messages_status_updated', 'processed_messages', ['status', 'updated_at'], unique=False)

def downgrade() -> None:
    op.drop_index('idx_processed_messages_status_updated', table_name='processed_messages')
