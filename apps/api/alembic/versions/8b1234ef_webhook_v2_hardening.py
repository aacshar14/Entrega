...
def upgrade():
    ...
    op.add_column('whatsapp_messages', sa.Column('phone_number_id', sa.String(), nullable=True))
    op.add_column('whatsapp_messages', sa.Column('sender_wa_id', sa.String(), nullable=True))
    op.add_column('whatsapp_messages', sa.Column('processing_status', sa.String(), server_default='pending', nullable=False))
    op.add_column('whatsapp_messages', sa.Column('processed_at', sa.DateTime(), nullable=True))
    op.add_column('whatsapp_messages', sa.Column('last_error', sa.Text(), nullable=True))
    
    # Indices for observability and reconciliation
    op.create_index('idx_whatsapp_messages_status', 'whatsapp_messages', ['processing_status'])
    op.create_index('idx_whatsapp_messages_tenant_status', 'whatsapp_messages', ['tenant_id', 'processing_status'])
