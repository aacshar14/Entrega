...
def upgrade() -> None:
    ...
    op.add_column('tenant_whatsapp_integrations', sa.Column('meta_app_id', sa.String(), nullable=True))
    op.add_column('tenant_whatsapp_integrations', sa.Column('configuration_id', sa.String(), nullable=True))
    op.add_column('tenant_whatsapp_integrations', sa.Column('business_account_id', sa.String(), nullable=True))
    
    # 2. Onboarding Status (Additive & Defaulted)
    op.add_column('tenant_whatsapp_integrations', sa.Column('onboarding_status', sa.String(), server_default='pending', nullable=False))
    ...
    res = conn.execute(sa.text("SELECT phone_number_id FROM tenant_whatsapp_integrations WHERE phone_number_id IS NOT NULL GROUP BY phone_number_id HAVING COUNT(*) > 1")).first()
    if res:
        # If duplicates exist, we can't add the constraint. For Level 2, we fail early with a clear message in logs.
        raise Exception(f"DATA INTEGRITY ERROR: Duplicate phone_number_id detected ({res[0]}). Cannot enforce uniqueness.")
    
    op.create_unique_constraint('uq_whatsapp_phone_number_id', 'tenant_whatsapp_integrations', ['phone_number_id'])


def downgrade() -> None:
    op.drop_constraint('uq_whatsapp_phone_number_id', 'tenant_whatsapp_integrations', type_='unique')
