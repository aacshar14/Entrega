...
def upgrade() -> None:
    ...
    op.add_column("tenant_whatsapp_integrations", sa.Column("token_expires_at", sa.DateTime(), nullable=True))
    op.add_column("tenant_whatsapp_integrations", sa.Column("display_phone_number", sqlmodel.sql.sqltypes.AutoString(), nullable=True))
    op.add_column("tenant_whatsapp_integrations", sa.Column("setup_completed", sa.Boolean(), server_default=sa.text("false"), nullable=True))
    ...
    conn = op.get_bind()
    res = conn.execute(sa.text("SELECT column_name FROM information_schema.columns WHERE table_name='tenant_whatsapp_integrations' AND column_name='encrypted_access_token'")).first()
    if res:
        op.alter_column('tenant_whatsapp_integrations', 'encrypted_access_token', new_column_name='access_token_encrypted')
