...
def test_dashboard_contract_shape():
    ...
    expected_stats_keys = {
        "customer_count", "product_count", "total_payments", 
        "total_debt", "low_stock_count", "weekly_produced", "weekly_delivered"
    }
    
    expected_stock_keys = {"name", "quantity", "quantity_outside", "total"}
    
    expected_activity_keys = {
        "id", "customer_name", "description", "quantity", "type", "amount", "created_at"
    }
    
    expected_billing_keys = {
        "status", "days_remaining", "is_expired", "trial_ends_at", "total_orders", "sales_today"
    }
