...
def test_webhook_routing_and_idempotency(db_session: Session):
    ...
    integration = TenantWhatsAppIntegration(
        tenant_id=tenant.id, phone_number_id="123456789", status="connected"
    )
    ...
    msg = db_session.exec(
        select(WhatsAppMessage).where(WhatsAppMessage.message_sid == "TEST_MSG_ID_001")
    ).first()
...
def test_parsing_delivery_and_payment():
    ...
    mock_prod = Product(
        sku="CHOC", name="Chocolate", price_menudeo=20.0, tenant_id=mock_tenant.id
    )
