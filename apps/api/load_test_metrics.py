...
def run_load_test():
    ...
    with Session(engine) as db:
        stmt = select(InboundEvent).where(InboundEvent.message_sid.like(f"wamid.{test_id}%")).where(InboundEvent.status == "pending")
    ...
    with Session(engine) as db:
        backlog = db.exec(select(func.count(InboundEvent.id)).where(InboundEvent.status == "pending")).one()
        print(f"\nQUEUE STATUS:")
