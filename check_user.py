...
def check():
    ...
    with engine.connect() as conn:
        ...
        try:
            result = conn.execute(
                text(
                    f"SELECT id, email, confirmed_at, last_sign_in_at FROM auth.users WHERE email = '{email}'"
                )
            )
        ...
        try:
            result = conn.execute(
                text(
                    f"SELECT id, full_name, role FROM profiles WHERE email = '{email}'"
                )
            )
            ...
            try:
                result = conn.execute(
                    text(
                        f"SELECT id, full_name, role FROM profiles_v2 WHERE email = '{email}'"
                    )
                )
                profile = result.fetchone()
